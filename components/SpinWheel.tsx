...
import { RefreshCw, Shuffle, Sparkles, Utensils, Coffee, Cookie } from 'lucide-react';
...
const SpinWheel: React.FC<Props> = ({ database, selectedItems, onSelect, onGenerateNewMenu, isGenerating, copy }) => {
  ...
  const handleSpinAll = () => {
    ...
    setTimeout(() => spinCategory('snack'), 200);
    setTimeout(() => spinCategory('drink'), 400);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="space-y-3">
          <button 
            ...
            className="w-full py-2.5 bg-orange-100 text-orange-700 font-bold rounded-xl hover:bg-orange-200 transition-colors text-sm flex items-center justify-center gap-2 shadow-sm"
          >
            <Utensils size={16}/> {copy.buttons.spin_now} Main Dish
          </button>
          <NutritionCard 
            category="Main Dish" 
        ...
        <div className="space-y-3">
          <button 
            ...
            className="w-full py-2.5 bg-yellow-100 text-yellow-700 font-bold rounded-xl hover:bg-yellow-200 transition-colors text-sm flex items-center justify-center gap-2 shadow-sm"
          >
            <Cookie size={16}/> {copy.buttons.spin_now} Snack
          </button>
          <NutritionCard 
            category="Snack" 
        ...
        <div className="space-y-3">
          <button 
            ...
            className="w-full py-2.5 bg-blue-100 text-blue-700 font-bold rounded-xl hover:bg-blue-200 transition-colors text-sm flex items-center justify-center gap-2 shadow-sm"
          >
            <Coffee size={16}/> {copy.buttons.spin_now} Drink
          </button>
          <NutritionCard 
            category="Drink" 
